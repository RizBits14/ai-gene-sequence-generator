    ...
    def alpha_beta(self, sequence, pool, maximizing, alpha, beta):
        ...
        if maximizing:
            ...
            for i in range(len(pool)):
                ...
                eval_score, candidate_sequence = self.alpha_beta(new_sequence, new_pool, False, alpha, beta)
            ...
            for i in range(len(pool)):
                ...
                new_sequence = sequence + nucleotide
                new_pool = pool[:i] + pool[i+1:]
                eval_score, candidate_sequence = self.alpha_beta(new_sequence, new_pool, True, alpha, beta)
...
target = "GCAT"
weights = [8, 8, 1, 1]
game = GeneSequenceGame(pool, target, weights)
utility_score, best_sequence = game.alpha_beta("", pool, True, float('-inf'), float('inf'))
